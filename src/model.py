...
class GoldPricePredictionModel:
    def __init__(self, X_train, X_test, y_train, y_test, units=128, dropout_rate=0.2, batch_size=32, epochs=200, learning_rate=0.001, scaler_y = MinMaxScaler(feature_range=(0, 1))):
    ...
    def build_model(self):
        ...
        # Input layer
        lstm_input = tf.keras.layers.Input(shape=(self.X_train.shape[1], 1), name="lstm_input")
        
        # First LSTM layer
        x = tf.keras.layers.LSTM(self.units, return_sequences=True, name="first_layer")(lstm_input)
        x = tf.keras.layers.Dropout(self.dropout_rate)(x)  # Dropout to reduce overfitting
        
        # Second LSTM layer (added layer)
        x = tf.keras.layers.LSTM(self.units // 2, return_sequences=False, name="second_layer")(x)
        x = tf.keras.layers.Dropout(self.dropout_rate)(x)  # Dropout after the second LSTM layer
    ...
    def plot_actual_vs_predicted(self):
        ...
        plt.plot(self.y_test_actual, label="Actual Gold Close Price", color='blue', alpha=0.6)
        plt.plot(self.y_pred, label="Predicted Gold Close Price", color='red', alpha=0.6)
